...
## Почему скорость не выросла в 2 раза (как на RTX 5080)

При переходе на более мощную карту (например RTX 5090) или при сравнении с 5080 скорость может вырасти не в 2 раза по нескольким причинам.

### 1. Occupancy (Blocks/SM)

Ядро собрано с `__launch_bounds__(256, 2)` и батчем 8 точек. При (256, 4) лимит 64 регистра вызывал сильный reg spilling → скорость падала в ~3 раза; 2 блока/SM без жёсткого лимита дают лучший throughput. Если **Blocks/SM: 1** или **2** — можно ещё поднять occupancy (см. ниже).

**Что смотреть:** при старте в логе выводится строка вида:
```text
[GPU 0] Device: ..., SMs: 170, Blocks/SM: 4 (fused kernel), ...
```
Чем выше Blocks/SM (при том же gridSize×blockSize), тем выше загрузка GPU.
...
Задача сильно нагружает память: SHA256, RIPEMD160, чтение/запись ключей. Пропускная способность памяти растёт не так, как число CUDA-ядер (у RTX 5090 ~1.87× от 5080 при ~2× ядер). Поэтому даже при идеальной загрузке ядер прирост часто **меньше 2×** — узкое место память, а не вычисления.
...
В `config.json` можно задать `gridSize` и `blockSize` (если не заданы — считаются по occupancy). На нестандартных картах иногда помогает подобрать значения вручную после просмотра лога (SMs, Blocks/SM, optimalGridSize).