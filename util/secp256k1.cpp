...
{
    const uint64_t sum64 = static_cast<uint64_t>(a) + b + carryIn;
    sum = static_cast<uint32_t>(sum64);
    carryOut = static_cast<int>(sum64 >> 32) & 1;
}

inline void subc(uint32_t a, uint32_t b, uint32_t borrowIn, uint32_t &diff, int &borrowOut)
{
    const uint64_t diff64 = static_cast<uint64_t>(a) - b - borrowIn;
    diff = static_cast<uint32_t>(diff64);
    borrowOut = static_cast<int>((diff64 >> 32) & 1);
}

static bool lessThanEqualTo(const uint32_t *a, const uint32_t *b, const int len = 8)
{
...
static bool greaterThanEqualTo(const uint32_t *a, const uint32_t *b, const int len = 8)
{
    for (int i = len - 1; i >= 0; i--)
    {
        if (a[i] > b[i])
...
static int add(const uint32_t *a, const uint32_t *b, uint32_t *c, const int len = 8)
{
    int carry = 0;
    for (int i = 0; i < len; i++)
    {
...
static int sub(const uint32_t *a, const uint32_t *b, uint32_t *c, const int len = 8)
{
    int borrow = 0;
    for (int i = 0; i < len; i++)
    {
...
static void multiply(const uint32_t *x, const int xLen, const uint32_t *y, const int yLen, uint32_t *z)
{
    ...
    //memset(z, 0, (yLen + xLen) * sizeof(uint32_t));
...
uint256 uint256::mul(uint64_t i) const
{
    ...
    uint32_t x[2];
    x[0] = static_cast<uint32_t>(i);
    x[1] = static_cast<uint32_t>(i >> 32);
    multiply(x, 2, v, 8, product);
    return {product};
}
...
{
    uint32_t product[16]{};
    multiply(&i, 1, v, 8, product);
    return {product};
}

uint256 uint256::div(const uint32_t val) const
{
    uint256 t = *this;
    uint256 quotient{};
...
{
    const uint256 quotient = div(val);
    const uint256 product = quotient.mul(val);
    uint256 result{};
    ::sub(v, product.v, result.v, 8);
    return result;
}

uint256 uint256::add(const int val) const
{
    uint256 result(val);
    ::add(v, result.v, result.v, 8);
    return result;
}
    ...
    while (!isOne(u) && !isOne(v))
    {
        while (isEven(u))
        {
            ...
            if (isEven(x1))
            {
                ...
                x1.v[7] |= (static_cast<uint32_t>(x1Signed) & 0x01) << 31;
                x1Signed >>= 1;
            }
            else
            {
                const int carry = add(x1.v, P.v, x1.v, 8);
                x1 = divBy2(x1);
                x1Signed += carry;
                x1.v[7] |= (static_cast<uint32_t>(x1Signed) & 0x01) << 31;
                x1Signed >>= 1;
        ...
        while (isEven(v))
        {
            ...
            {
                x2 = divBy2(x2);
                x2.v[7] |= (static_cast<uint32_t>(x2Signed) & 0x01) << 31;
                x2Signed >>= 1;
            }
            else
            {
                const int carry = add(x2.v, P.v, x2.v, 8);
                x2 = divBy2(x2);
                x2Signed += carry;
                x2.v[7] |= (static_cast<uint32_t>(x2Signed) & 0x01) << 31;
                x2Signed >>= 1;
        ...
        if (lessThanEqualTo(v.v, u.v, 8))
        {
            sub(u.v, v.v, u.v, 8);
            // x1 = x1 - x2
            const int borrow = sub(x1.v, x2.v, x1.v, 8);
            x1Signed -= x2Signed;
            x1Signed -= borrow;
        }
        else
...
{
    uint256 sum;
    const int overflow = add(a.v, b.v, sum.v, 8);
    // mod P
...
uint256 secp256k1::multiplyModP(const uint256 &a, const uint256 &b)
{
    ...
    add(tmp, tmp2, tmp, 10);
    // add to low 8 words
    const int overflow = add(&product[0], &tmp[0], &product[0], 8);
...
static void reduceModN(const uint32_t *x, uint32_t *r)
{
    const uint32_t barrettN[]{0x2fc9bec0, 0x402da173, 0x50b75fc4, 0x45512319, 0x00000001, 0x00000000, 0x00000000, 0x00000000, 00000001};
    uint32_t product[25]{};
...
std::string secp256k1::uint256::toString(int) const
{
    std::string s;
    for (int i = 7; i >= 0; i--)
    {
...
{
    if (std::any_of(std::begin(p.x.v), std::end(p.x.v), [](uint32_t value) { return value != 0xFFFFFFFF;}))
        return false;

    if (std::any_of(std::begin(p.y.v), std::end(p.y.v), [](uint32_t value) { return value != 0xFFFFFFFF;}))
        return false;
...
ecpoint secp256k1::addPoints(const ecpoint &p1, const ecpoint &p2)
{
    ...
    const uint256 rise = subModP(p1.y, p2.y);
    const uint256 run = subModP(p1.x, p2.x);
    const uint256 s = multiplyModP(rise, invModP(run));
    //rx = (s*s - px - qx) % _p;
    const uint256 rx = subModP(subModP(multiplyModP(s, s), p1.x), p2.x);
    //ry = (s * (px - rx) - py) % _p;
    const uint256 ry = subModP(multiplyModP(s, subModP(p1.x, rx)), p1.y);
    return {rx, ry}; // sum
}
...
static void bulkInversionModP(std::vector<uint256> &in)
{
    ...
    for (const auto &i: in)
    {
        total = secp256k1::multiplyModP(total, i);
...
void secp256k1::generateKeyPairsBulk(uint32_t count, const ecpoint &basePoint, std::vector<uint256> &privKeysOut, std::vector<ecpoint> &pubKeysOut)
{
    privKeysOut.clear();
...
{
    const auto count = privKeys.size();
    //privKeysOut.clear();
    pubKeysOut.clear();
    ...
    for (int i = 0; i < 256; i++)
    {
        ...
        {
            uint256 run;
            if (uint256 k = privKeys[j]; k.bit(i))
            {
...
bool secp256k1::checkECKeyWithLibSecp256k1(uint256 &privKey, ecpoint &pubKey)
{
    // std::string privateKeyHex = "0100000000000000000000000000000000000000000000000000000000000200";
    // uint256 pKey = uint256(privateKeyHex);
    // std::cout << pKey.toString() << std::endl;
    //
    // unsigned char pKeyExported[32]{};
    // pKey.exportWords(reinterpret_cast<uint32_t*>(pKeyExported), sizeof(pKeyExported) / sizeof(uint32_t), 1);
    // for (unsigned char i : pKeyExported)
    // {
    //     std::cout << std::hex << std::setw(2) << std::setfill('0') << static_cast<int>(i);
    // }
    // std::cout << std::endl;
    //
    // // Initialize the secp256k1 context for signing (and optionally verification)
    // secp256k1_context *ctx = secp256k1_context_create(SECP256K1_CONTEXT_NONE);
    // ScopeOutRunner outRunner([ctx] { if (ctx) { secp256k1_context_destroy(ctx); } });
    //
    // // Derive the public key from the private key
    // secp256k1_pubkey publicKey;
    // if (!secp256k1_ec_pubkey_create(ctx, &publicKey, reinterpret_cast<const unsigned char *>(pKeyExported)))
    // {
    //     std::cerr << "Failed to create public key.\n";
    //     return false;
    // }
    // // Serialize the public key in compressed format
    // constexpr int compressed = SECP256K1_EC_UNCOMPRESSED;
    // size_t pubKeyLen = (compressed == SECP256K1_EC_COMPRESSED) ? 33 : 65 /*SECP256K1_EC_UNCOMPRESSED*/;
    // unsigned char serializedPubKey[65]{};
    // secp256k1_ec_pubkey_serialize(ctx, serializedPubKey, &pubKeyLen, &publicKey, compressed);
    //
    ...
    //     std::cout << std::hex << std::setw(2) << std::setfill('0') << (int)serializedPubKey[i];
    // }
    // std::cout << std::endl;
    // std::cout << "\n";
    // // Clean up
