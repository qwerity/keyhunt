...
HttpClient::HttpClient(const ServerConfig& config) : mConfig(config), mIOContext(), mStream(mIOContext)
{
...
http::status HttpClient::get(const std::string& target, http::response<http::dynamic_body>& response)
{
    ...
    try
    {
        ...
        auto const results = resolver.resolve(mConfig.url, mConfig.port);

        // Connect to the server
        mStream.connect(results);
        ...
        request.set(http::field::host, mConfig.url);
        request.set(http::field::user_agent, BOOST_BEAST_VERSION_STRING);
        request.set(http::field::authorization, mConfig.authorisationHeader);

        // Send the request
        http::write(mStream, request);

        // Buffer is used to read raw network data
        beast::flat_buffer buffer;

        // Receive the HTTP response
        http::read(mStream, buffer, response);
