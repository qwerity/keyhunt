...
namespace
{
    ...
    struct SecureRandomState
    {
        ...
        int nextBIndex{HASHBYTES_TO_USE};
        uint64_t counter{0};
        int firstCall{1};
    };

    void initSecureRandomState(SecureRandomState* state, uint32_t x, uint32_t y)
    {
        state->nextBIndex = HASHBYTES_TO_USE;
        state->counter = 0;
        state->firstCall = 1;

        std::fill_n(state->seed, HASH_OFFSET + EXTRAFRAME_OFFSET, 0u);
        std::fill_n(state->nextBytes, DIGEST_LENGTH, static_cast<uint8_t>(0));
    ...
    void nextBytes(SecureRandomState* state, uint8_t* bytes, int bytesLen)
    {
        ...
        {
            const int remaining = HASHBYTES_TO_USE - state->nextBIndex;
            const int n = std::min(remaining, bytesLen - nextByteToReturn);
            for (int i = 0; i < n; ++i)
            {
        ...
        {
            state->seed[lastWord] = static_cast<uint32_t>(counter >> 32);
            state->seed[lastWord + 1] = static_cast<uint32_t>(counter & 0xFFFFFFFF);
            state->seed[lastWord + 2] = END_FLAG;

            computeHash(state->seed);
            counter++;
            ...
            {
                const uint32_t k = h[i];
                const int offset = i * 4;
                nb[offset] = static_cast<uint8_t>(k >> 24);
                nb[offset + 1] = static_cast<uint8_t>(k >> 16);
                nb[offset + 2] = static_cast<uint8_t>(k >> 8);
                nb[offset + 3] = static_cast<uint8_t>(k);
            }

            state->nextBIndex = 0;
            const int bytesToCopy = std::min(HASHBYTES_TO_USE, bytesLen - nextByteToReturn);
...
void crypto::generatePrivateKey2(int32_t x, int32_t y, uint8_t* output)
{
    constexpr int numBits = 256;
    constexpr int numberLength = (numBits + 31) >> 5;

    SecureRandomState state;
    initSecureRandomState(&state, static_cast<uint32_t>(x), static_cast<uint32_t>(y));
    ...
    for (int i = 0; i < numberLength; ++i)
    {
        ...
        output[offset] = static_cast<uint8_t>(digits[i] >> 24);
        output[offset + 1] = static_cast<uint8_t>(digits[i] >> 16);
        output[offset + 2] = static_cast<uint8_t>(digits[i] >> 8);
        output[offset + 3] = static_cast<uint8_t>(digits[i]);
    }
}
