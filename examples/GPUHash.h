...
// ---------------------------------------------------------------------------------
// MD5 (for Android 4.0 vulnerability simulation)
// ---------------------------------------------------------------------------------

__device__ __constant__ uint32_t MD5_K[] = {
...
__device__ __constant__ uint32_t MD5_S[] = {
...
__device__ __constant__ uint32_t MD5_INIT_A = 0x67452301;
__device__ __constant__ uint32_t MD5_INIT_B = 0xefcdab89;
__device__ __constant__ uint32_t MD5_INIT_C = 0x98badcfe;
__device__ __constant__ uint32_t MD5_INIT_D = 0x10325476;
...
#define MD5_LEFTROTATE(value, amount) (((value) << (amount)) | ((value) >> (32 - (amount))))

__device__ void _MD5Transform(uint32_t h[4], const uint8_t data[64]) {
    uint32_t w[16];
    uint32_t a = h[0], b = h[1], c = h[2], d = h[3];

    // Prepare message schedule
    for (int i = 0; i < 16; i++) {
        w[i] = (uint32_t)(data[i*4]) | ((uint32_t)(data[i*4+1]) << 8) |
...
__device__ void _MD5Hash(uint8_t output[16], const uint8_t* input, size_t input_len) {
    uint32_t h[4] = {MD5_INIT_A, MD5_INIT_B, MD5_INIT_C, MD5_INIT_D};
    uint8_t padded[64];
    size_t total_len = input_len + 9; // +1 for 0x80, +8 for length

    // Process input in 512-bit chunks
    size_t pos = 0;
    while (pos + 64 <= input_len) {
        _MD5Transform(h, input + pos);
        pos += 64;
    }

    // Handle remaining data
    size_t remaining = input_len - pos;
    memcpy(padded, input + pos, remaining);
    padded[remaining] = 0x80;

    if (remaining >= 56) {
        // Need another block
        memset(padded + remaining + 1, 0, 64 - remaining - 1);
        _MD5Transform(h, padded);

        memset(padded, 0, 56);
    } else {
        memset(padded + remaining + 1, 0, 56 - remaining);
    }
    ...
    // Output hash as little-endian bytes
    for (int i = 0; i < 4; i++) {
        output[i*4] = h[i] & 0xFF;
