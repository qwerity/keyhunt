...
__device__ __forceinline__ uint32_t F(const uint32_t x, const uint32_t y, const uint32_t z)
{
    return x ^ y ^ z;
}

__device__ __forceinline__ uint32_t G(const uint32_t x, const uint32_t y, const uint32_t z)
{
    return (((x) & (y)) | (~(x) & (z)));
}

__device__ __forceinline__ uint32_t H(const uint32_t x, const uint32_t y, const uint32_t z)
{
    return (((x) | ~(y)) ^ (z));
}

__device__ __forceinline__ uint32_t I(const uint32_t x, const uint32_t y, const uint32_t z)
{
    return (((x) & (z)) | ((y) & ~(z)));
}

__device__ __forceinline__ uint32_t J(const uint32_t x, const uint32_t y, const uint32_t z)
{
    return ((x) ^ ((y) | ~(z)));
}

__device__ __forceinline__ void FF(uint32_t &a, const uint32_t b, uint32_t &c, const uint32_t d, const uint32_t e, const uint32_t x, const uint32_t s)
{
    a += F(b, c, d) + x;
    a = rotl(a, s) + e;
    c = rotl(c, 10);
...
{
    a += J(b, c, d) + x + RIPEMD160_K3;
    a = rotl(a, s) + e;
    c = rotl(c, 10);
}

__device__ __forceinline__ void FFF(uint32_t &a, const uint32_t b, uint32_t &c, const uint32_t d, const uint32_t e, const uint32_t x, const uint32_t s)
{
    a += F(b, c, d) + x;
    a = rotl(a, s) + e;
    c = rotl(c, 10);
...
{
    a += J(b, c, d) + x + RIPEMD160_K7;
    a = rotl(a, s) + e;
    c = rotl(c, 10);
}

__device__ __forceinline__ void ripemd160sha256(const uint32_t x[8], uint32_t digest[5])
{
    uint32_t a1 = RIPEMD160_IV0;
    uint32_t b1 = RIPEMD160_IV1;
    uint32_t c1 = RIPEMD160_IV2;
    uint32_t d1 = RIPEMD160_IV3;
    uint32_t e1 = RIPEMD160_IV4;

    constexpr uint32_t x8 = 0x00000080;
    constexpr uint32_t x14 = 256;
    ...
    uint32_t a2 = RIPEMD160_IV0;
    uint32_t b2 = RIPEMD160_IV1;
    uint32_t c2 = RIPEMD160_IV2;
    uint32_t d2 = RIPEMD160_IV3;
    uint32_t e2 = RIPEMD160_IV4;
    /* parallel round 1 */
    ...
    digest[0] = RIPEMD160_IV1 + c1 + d2;
    digest[1] = RIPEMD160_IV2 + d1 + e2;
